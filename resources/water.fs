...
float calcShadow(vec4 FragPos, vec3 normal, vec3 sunDir) {
    ...
    float bias = max(0.05 * (1.0 - abs(diff)), 0.005);
...
void main() {
    ...
    // texCoordRef = clamp(texCoordRef + distortion, 0.001, 0.999); // 扰动
    vec3 refractColor = texture(refractionMap, texCoordRef).rgb;
    vec3 reflectColor = texture(reflectionMap, texCoordRef).rgb;

    vec3 blueColor = vec3(0.0, 0.25, 1.0); // 假装这是纹理采出来的

    // 0.4 * refractColor + 0.4 * reflectColor + 0.2 * blueColor
    // mix(A, B, factor) = A * (1 - factor) + B * factor
    vec3 color = mix(mix(refractColor, reflectColor, 0.4), blueColor, 0.2);
    ...
    vec3 specular = vec3(0.0); // no specular so far
