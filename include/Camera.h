...
public:
    ...
    void ProcessMouseScroll(float yoffset) { Zoom = CLAMP(Zoom - (float)yoffset, 1.0f, 45.0f); }

    // returns the projection and view matrix calculated using Zoom value, Euler Angles and the LookAt Matrix
    glm::mat4 GetPerspectiveMatrix() { return glm::perspective(glm::radians(Zoom), SCR_SCALE, Near, Far); }
    glm::mat4 GetOrthoMatrix(float left, float right, float bottom, float top) { return glm::ortho(left, right, bottom, top, Near, Far); }
