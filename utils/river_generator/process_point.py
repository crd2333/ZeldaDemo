    ...
    if event.xdata is not None and event.ydata is not None:
        ...
        if base_point is None:  # 第一次点击，记录基准点
            ...
            if len(points) > 1:
                ...
                ax.plot([x * width for x in x_vals], [(1 - y) * height for y in y_vals], 'r-')
            fig.canvas.draw()
...
print("float points[] = {")
for point in points:
    print(f"    {point[0] - 0.5:.6f}f, {point[1] - 0.5:.6f}f, {point[2]:.6f}f, {point[3]:.6f}f,")
print("};")